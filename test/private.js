...
describe('/private', () => {
    ...
    it('returns a greeting for the authenticated user', (done) => {
        ...
        University.init(0, (err, server) => {
            ...
            const request = { method: 'GET', url: '/private', headers: { authorization: internals.header('admin', Users.admin.password) } };
            server.inject(request, (res) => {

                expect(res.statusCode, 'StatusCode').to.equal(200);
                expect(res.result, 'result').to.equal('<div>Hello admin</div>');
